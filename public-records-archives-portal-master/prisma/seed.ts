...
async function main() {
    ...
    const archivist = await prisma.user.upsert({
        where: { email: 'p.chikwanda@archive.gov.zw' },
        update: {},
        create: {
            username: 'p.chikwanda',
            email: 'p.chikwanda@archive.gov.zw',
            fullName: 'Patricia Chikwanda',
            password: 'archivist123',
            role: 'PROCESSING_ARCHIVIST',
            department: 'Processing Unit',
            status: 'active',
            employeeId: 'EMP-PROC-02',
            accessControl: 'SECRET',
            mfaEnabled: false,
        },
    })

    const digitizationTech = await prisma.user.upsert({
        where: { email: 't.moyo@archive.gov.zw' },
        update: {},
        create: {
            username: 't.moyo',
            email: 't.moyo@archive.gov.zw',
            fullName: 'Takudzwa Moyo',
            password: 'tech123',
            role: 'DIGITIZATION_TECH',
            department: 'Digitization Unit',
            status: 'active',
            employeeId: 'EMP-DIGI-03',
    ...
    const records = [
        ...
        // ── IMAGE ──
        {
            ...
            type: 'IMAGE',
            status: 'approved',
            department: 'Heritage & Conservation',
        ...
        },
        {
            ...
            type: 'IMAGE',
            status: 'approved',
            department: 'State House Archives',
            collectionId: independence.id,
        ...
        // ── MAP ──
        {
            ...
            type: 'MAP',
            status: 'approved',
            department: 'Cartographic Records',
            collectionId: colonial.id,
        ...
        // ── VIDEO ──
        {
            ...
            type: 'VIDEO',
            status: 'approved',
            department: 'Film & Broadcast Archives',
            collectionId: independence.id,
    ...
    const auditEntries = [
        { userId: archivist.id, action: 'RECORD_UPLOADED', resource: 'Record', details: JSON.stringify({ ref: 'REF-2025-004' }) },
        { userId: archivist.id, action: 'METADATA_UPDATED', resource: 'Record', details: JSON.stringify({ ref: 'IMG-2025-001' }) },
        { userId: digitizationTech.id, action: 'RECORD_UPLOADED', resource: 'Record', details: JSON.stringify({ ref: 'AUD-2025-001' }) },
        { userId: digitizationTech.id, action: 'RECORD_UPLOADED', resource: 'Record', details: JSON.stringify({ ref: 'VID-2025-001' }) },
        { userId: digitizationTech.id, action: 'METADATA_UPDATED', resource: 'Record', details: JSON.stringify({ ref: 'MAP-2025-001' }) },
